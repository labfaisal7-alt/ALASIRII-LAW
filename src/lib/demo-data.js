...
let serviceRequestsStore = [
    {
        ...
        details: "طلب خدمة مبدئي لمراجعة عقد توريد وبدء إجراءات المطالبة بالمستحقات.",
        source: "website",
        priority: "high",
        status: "in_review",
    ...
    },
    {
        ...
        details: "العميل يريد تقييم الموقف القانوني قبل رفع دعوى بخصوص تأخير واعتماد مستخلصات.",
        source: "referral",
        priority: "urgent",
        status: "converted",
...
let casesStore = [
    {
        ...
        description: "مطالبة مالية ناشئة عن عقد مقاولة وتأخر اعتماد المستخلصات مع مراسلات تمهيدية قائمة.",
        hearings: [
            {
                id: "hearing-case-001",
    ...
    },
    {
        ...
        branchName: "فرع الرياض",
        description: "مطالبة بمستحقات عمالية وبدل إجازات مع مستندات ابتدائية مكتملة.",
        hearings: [],
        deadlines: [],
        filings: [
            {
...
const delay = (ms = 120) => new Promise((resolve) => {
    window.setTimeout(resolve, ms);
});
const clone = (value) => JSON.parse(JSON.stringify(value));
const toCaseListItem = (item) => ({
    id: item.id,
...
const toServiceRequestListItem = (item) => ({
    id: item.id,
...
const buildDashboard = () => ({
    metrics: [
        ...
        {
            label: "مدد حرجة",
            value: String(deadlinesStore.filter((item) => item.status === "pending" && item.daysLeft <= 4)
                .length),
            tone: "danger"
        }
    ],
...
export const demoApi = {
    async login(input) {
        await delay();
        const validEmail = input.email === DEMO_USER.email || input.email === "admin@lawoffice.sa";
    ...
    async convertServiceRequestToCase(id) {
        await delay();
